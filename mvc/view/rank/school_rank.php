<?php 
    include "../../controller/authenticator/auth.php";
    include "../../model/fungsi.php";

    $id = $_SESSION['id_user'];
    $query = mysqli_query($conn, "SELECT * FROM user WHERE id_user = $id");
    $hasil = mysqli_fetch_all($query, MYSQLI_ASSOC)[0];

    $hasil2 = [];
    $hasil3 = [];
    if(isset($_POST['submit'])) {
        $hasil4 = cariSekolah(htmlspecialchars($_POST['sekolah']));
        $hasil2 = tampilRangkingSekolah($hasil4['id_sekolah']);
        $hasil3 = tampilNamaSekolah($hasil4['id_sekolah']);
    }
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../../controller/css/style.css">
    <title>Add Account</title>
</head>
<body>
    <div class="header">
        <div class="left">
            <h1>Flashcard</h1>
            <a href="../dashboard/dashboard.php">Home</a>
            <a href="../play/play.php">Play Flashcard</a>
            <div class="menu">Menu
                <div>
                    <a href="../flashcard_deck/flashcard_deck.php">My Flashcard Deck</a>
                    <a href="../history/history.php">History</a>
                </div>
            </div>
            <?php if($hasil['role'] == "admin"): ?>
                <div class="menu">Management
                    <div>
                        <a href="../account/account.php">Account</a>
                        <a href="../school/school.php">School</a>
                    </div>
                </div>
            <?php endif; ?>
        </div>
        <div class="right">
            <div class="user">
                <?php echo $hasil['username'] ?>
                <div>
                    <a href="../account/edit_account.php">Edit My Account</a>
                    <a href="../../controller/authenticator/logout.php">logout</a>
                </div>
            </div>
        </div>
    </div>
    <div class="content">
        <h1>School Rangking</h1>
        <div class="rangking">
            <a href="../dashboard/dashboard.php">Back</a>
            <form action="" method="POST">
                <input list="sekolahs" name="sekolah" placeholder="Search your school">
                <datalist id="sekolahs">
                    <?php foreach($data_sekolah as $d): ?>
                        <option value="<?php echo $d['nama_sekolah'] ?>"></option>
                    <?php endforeach; ?>
                </datalist>
                <button type="submit" name="submit" value="submit">Check</button>
                <input type="text" id="filter" placeholder="Search your name">
            </form>
        </div>
        <table id="table">
            <tr>
                <th>No</th>
                <th>Username</th>
                <th>Score</th>
            </tr>
            <?php $num = 1 ?>
            <?php foreach($hasil2 as $h): ?>
                <td><?php echo $num++ ?></td>
                <td><?php echo $h['username'] ?></td>
                <td><?php echo $h['total_skor'] ?></td>
            <?php endforeach; ?>
        </table>
    </div>
    <script>
        const filter = document.getElementById("filter")
        let table = document.getElementById("table")
        
        filter.addEventListener("keyup", () => {
            let keyword = filter.value.toLowerCase();

            for (let index = 1; index < table.rows.length; index++) {
                const rowText = table.rows[index].textContent.toLowerCase();

                if (rowText.includes(keyword)) {
                    table.rows[index].style.display = "";
                } else {
                    table.rows[index].style.display = "none";
                }
            }
        });
    </script>
</body>
</html>